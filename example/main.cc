...
inline void MultiArrayExample(Client& client) {
  ...
  auto a2 = std::make_shared<ColumnArray>(
  ...
  client.Select("SELECT arr FROM test_multiarray",
      [](const Block& block) {
        for (size_t c = 0; c < block.GetRowCount(); ++c) {
          auto col = block[0]->As<ColumnArray>()->GetAsColumn(c);
          cout << "[";
...
inline void GenericExample(Client& client) {
  ...
  client.Select("SELECT id, name FROM psi_data limit 1", [](const Block& block) {
    // std::cout << PrettyPrintBlock{block} << std::endl;
      for (size_t i = 0; i < block.GetRowCount(); i++) {
        std::cout << "id: " << block[0]->As<ColumnString>()->At(i) << " "
            << "name: " << block[1]->As<ColumnString>()->At(i) << "\n";
      }
    }
  );
...
inline void NumbersExample(Client& client) {
  ...
  client.Select("SELECT number, number FROM system.numbers LIMIT 10",
    [&num](const Block& block) {
  ...
  client.Select("SELECT id, v4, v6 FROM test_ips", [](const Block& block)
    {
      ...
      for (size_t i = 0; i < block.GetRowCount(); ++i) {
        std::cout << (*block[0]->As<ColumnUInt64>())[i] << " "
            << (*block[1]->As<ColumnIPv4>()).AsString(i)
            << " (" << (*block[1]->As<ColumnIPv4>())[i].s_addr << ") "
...
static void RunTests(Client& client) {
    std::cout << "ArrayExample" << std::endl;
    ArrayExample(client);
    // std::cout << "CancelableExample" << std::endl;
    // CancelableExample(client);
    // std::cout << "DateExample" << std::endl;
    // DateExample(client);
    // DateTime64Example(client);
    std::cout << "DecimalExample" << std::endl;
    DecimalExample(client);
    // EnumExample(client);
    // ExecptionExample(client);
    std::cout << "GenericExample" << std::endl;
    GenericExample(client);
    std::cout << "IPExample" << std::endl;
    IPExample(client);
    std::cout << "MultiArrayExample" << std::endl;
    MultiArrayExample(client);
    // NullableExample(client);
    std::cout << "NumbersExample" << std::endl;
...
int main() {
  try {
    const auto localHostEndpoint = ClientOptions().SetHost("172.21.1.58")
                                                  .SetPort(9000)
                                                  .SetUser("default")
                                                  .SetPassword("primihub")
    ...
    //     RunTests(client);
    //     std::cout << "current endpoint : "
    //               <<  client.GetCurrentEndpoint().value().host << "\n";
    // }
